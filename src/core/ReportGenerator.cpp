...
QString ReportGenerator::generateServiceReportHtml(const QString &targetFilePath)
{
    ...
    ts << QString::fromUtf8(
       ...
       << QString::fromUtf8("    <div class=\"info-card\"><strong>Baza sygnatur</strong><span>").toUtf8() << (SignatureDb::instance().totalSignatures() > 0 ? QString::number(SignatureDb::instance().totalSignatures()) + QString::fromUtf8(" sygnatur") : QString::fromUtf8("Zainicjalizowana")).toUtf8() << "</span></div>\n"
       << "  </div>\n\n"
       << QString::fromUtf8(
       "  <h2>2. Status Aktywnych Osłon Ochronnych</h2>\n"
       "  <table>\n"
       "    <tr><th>Komponent Ochrony</th><th>Opis modułu</th><th>Status</th></tr>\n"
       "    <tr><td>Ochrona w czasie rzeczywistym (Real-Time Shield)</td><td>Canary Guard, heurystyczny analizator plików</td><td><span class=\"badge ").toUtf8() << (Settings::instance().realTimeProtection() ? "badge-success\">AKTYWNA" : "badge-warn\">WYŁĄCZONA") << "</span></td></tr>\n"
       << QString::fromUtf8("    <tr><td>Ochrona przed Ransomware (Controlled Folders)</td><td>Monitorowanie szyfrowania i obrona dokumentów</td><td><span class=\"badge ").toUtf8() << (Settings::instance().ransomwareProtection() ? "badge-success\">AKTYWNA" : "badge-warn\">WYŁĄCZONA") << "</span></td></tr>\n"
       << QString::fromUtf8("    <tr><td>Ochrona sieciowa &amp; filtrowanie DNS (Web &amp; DNS Shield)</td><td>Blokowanie domen phishingowych, ransomware C2 i szkodliwych adresów DNS</td><td><span class=\"badge ").toUtf8() << (Settings::instance().webShield() ? "badge-success\">AKTYWNA" : "badge-warn\">WYŁĄCZONA") << "</span></td></tr>\n"
       << QString::fromUtf8("    <tr><td>Ochrona zapytań DNS (DNS Threat Protection)</td><td>Zaawansowana ochrona przed zatruwaniem bufora DNS i fałszywym routingiem</td><td><span class=\"badge ").toUtf8() << (Settings::instance().webShield() ? "badge-success\">AKTYWNA" : "badge-warn\">WYŁĄCZONA") << "</span></td></tr>\n"
       << QString::fromUtf8("    <tr><td>Skaner nośników wymiennych (USB Sentinel)</td><td>Automatyczna analiza nośników pendrive / dysków USB</td><td><span class=\"badge ").toUtf8() << (Settings::instance().scanUsbOnInsert() ? "badge-success\">AKTYWNA" : "badge-warn\">WYŁĄCZONA") << "</span></td></tr>\n"
       << "  </table>\n\n"
       << QString::fromUtf8(
